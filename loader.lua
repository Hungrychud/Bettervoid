-- BetterVoid standalone merged loader.
-- Includes the recreated mon-2d routes for regular executors and embeds the
-- patched BetterVoid main script so Matcha does not need readfile/HttpGet.

local env = getgenv and getgenv() or _G
if env.BETTERVOID_LOADER_LOADED then
    return
end
env.BETTERVOID_LOADER_LOADED = true

local function betterVoidLoaderIsMatcha()
...
local function betterVoidLoaderRunUrl(url)
    local chunk, err = loadstring(game:HttpGet(url))
    if not chunk then
        error("BetterVoid loader failed to compile " .. tostring(url) .. ": " .. tostring(err))
    end
    return chunk()
end

if not betterVoidLoaderIsMatcha() then
    local monLoaders = {
        GameId = {
            [1008451066] = "https://api.luarmor.net/files/v4/loaders/ff0850aeaef1c7f649d5bd3acf995976.lua",
            [6035872082] = "https://api.luarmor.net/files/v4/loaders/820752256ceb15cff17a05fd607aaf8b.lua",
        },
        PlaceId = {
            [155615604] = "https://api.luarmor.net/files/v4/loaders/4e79dc4ab9adddae25279dc271aa9e97.lua",
            [114234929420007] = "https://api.luarmor.net/files/v4/loaders/27a3877e7273ed8c9cf12d2bd25c0e11.lua",
        },
        Default = "https://api.luarmor.net/files/v4/loaders/5e0225ad1ab8668fb102acc2a4e64bc9.lua",
    }

    local gameLoader = monLoaders.GameId[game.GameId]
    if gameLoader then
        betterVoidLoaderRunUrl(gameLoader)
    end

    local placeLoader = monLoaders.PlaceId[game.PlaceId]
    if placeLoader then
        betterVoidLoaderRunUrl(placeLoader)
    end

    betterVoidLoaderRunUrl(monLoaders.Default)
end

-- Embedded BetterVoid main.lua starts here.
local function boot()
local Players
local lp
...
-- Credits:
-- BetterVoid GUI/scripts: Hungrychud/Bettervoid
-- Original recreated loader routes: mon-2d/loader
-- Matcha compatibility merge: ChatGPT