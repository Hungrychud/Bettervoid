local function boot()
    ...
    local function createUi()
        ...
        handles.height       = controls:Slider("Height",        S.height,       1000, 100,  100000, " studs", function(v) S.height = math.floor(v) end)
        handles.rate         = controls:Slider("Tick delay",    S.rate,         0.01, 0.03, 0.5,    "s",       function(v) S.rate = math.max(0.03, v) end)
        handles.velocity     = controls:Slider("Up velocity",   S.velocity,     100,  500,  5000,   "",        function(v) S.velocity = math.floor(v) end)
        handles.returnHeight = controls:Slider("Return height", S.returnHeight, 1,    0,    50,     " studs",  function(v) S.returnHeight = math.floor(v) end)

        roamToggle         = controls:Toggle("Map roam", S.roam, function(on) S.roam = on and true or false end)
        handles.roamRadius = controls:Slider("Roam radius", S.roamRadius, 50,   100, 5000,   " studs", function(v) S.roamRadius = math.floor(v) end)
        handles.roamSpeed  = controls:Slider("Roam speed",  S.roamSpeed,  1000, 0.1, 100000, "",        function(v) S.roamSpeed = math.max(0.1, math.min(100000, v)) end)

        controls:Divider("Shooting support")
        controls:Toggle("Aim stabilizer", S.aimStabilizer, function(on) S.aimStabilizer = on and true or false end)
        handles.aimHoldTime = controls:Slider("Aim hold time", S.aimHoldTime, 0.05, 0.05, 3, "s", function(v) S.aimHoldTime = math.max(0.05, math.min(3, v)) end)
        overlayToggle = controls:Toggle("Position overlay", S.showOverlay, function(on) S.showOverlay = on and true or false end)

        controls:Divider("Armor")
        autoArmorToggle = controls:Toggle("Auto armor assist", S.autoArmor, function(on) S.setAutoArmor(on) end)
        handles.armorCooldown     = controls:Slider("Armor cooldown", S.armorCooldown,     1,    2,   60, "s",  function(v) S.armorCooldown = math.floor(v) end)
        handles.armorTriggerRatio = controls:Slider("Buy below armor", S.armorTriggerRatio, 0.05, 0.1, 1, "",   function(v) S.armorTriggerRatio = math.max(0.1, math.min(1, v)) end)
        ...
        status:Label(function() return "Kill status: " .. tostring(S.killAllStatus) end)
        ...
        status:Info("P menu, V void, K kill all, L selected kill, reset key, scroll = cycle target.")

        -- INSTANT KILL TAB — Controls
        ...
        killControls:Button("Find people", function() openPeopleFinder() end)
        handles.peopleFinderSection:Button("Refresh people list", function() openPeopleFinder() end)
        ...
        autoKillTargetToggle = killControls:Toggle("Auto kill selected", S.autoKillTarget, function(on)
            S.autoKillTarget = on and true or false
        ...
        -- INSTANT KILL TAB — Status
        killStatus:Label(function() return "Target: " .. tostring(S.killTargetName or "none") end)
        killStatus:Label(function() return "Typed: " .. tostring(S.killTargetInput or "") end)
        ...
        killStatus:Label(function()
            ...
            return "Health: " .. math.floor(tonumber(hum.Health) or 0) .. "/" .. math.floor(tonumber(hum.MaxHealth) or 100)
        end)
        killStatus:Label(function()
            local mode = S.killOnSight and "sight" or S.autoKillAll and "all" or S.autoKillTarget and "target" or "off"
            return "Auto kill: " .. mode
        end)
        killStatus:Label(function() return "Interval: " .. tostring(S.autoKillInterval) .. "s" end)
        killStatus:Info("Mouse wheel cycles targets. K=kill all, L=kill selected.")
